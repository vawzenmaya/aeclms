...
class LoanFormPdfService {
  ...
  static Future<void> generateInitialApplicationForm({
    ...
    // 1. Package data for the background thread
    final params = {
      'loan': loan,
      'stages': stages ?? [],
      'actions': actions ?? [],
    ...
    // 2. Offload PDF generation to a background core to prevent freezing
    final Uint8List pdfBytes = await compute(_buildInitialFormTask, params);

    // 3. Print the generated bytes
    await Printing.layoutPdf(
      onLayout: (PdfPageFormat format) async => pdfBytes,
      name: 'Loan_Application_${loan['full_name']?.replaceAll(' ', '_') ?? 'Draft'}.pdf',
    );
  }
